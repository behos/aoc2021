...
type Point = (Int, Int)
data PointState = PointState { checked :: Bool
                             , val     :: Int
                             } deriving Show
data Board = Board { numPoints   :: HashMap.HashMap Int Point
...
boardFromString content = do
  let rows = lines content
  let vec = Lib.toIntList . words <$> rows
  boardFromVec $ asPointPairs 0 0 vec
...
boardFromVec [] = Board { numPoints=HashMap.empty
                        , pointStates=HashMap.empty
                        }
boardFromVec ((point, x):xs) = do
  let board = boardFromVec xs
  board { numPoints = HashMap.insert x point $ numPoints board
        , pointStates =
            HashMap.insert point PointState { checked=False
                                            , val=x
                                            } $ pointStates board
        }
...
winningBoards [] _ = error "no all boards won"
winningBoards _ [] = []
winningBoards (x:xs) boards = do
  let upd = processNumber x <$> boards
  let (winners, losers) = partition (wonWithNumber x) upd
  map (x,) winners ++ winningBoards xs losers


processNumber :: Int -> Board -> Board
processNumber x board = do
  case numPoints board !? x of
    Nothing -> board
    Just point ->
      board { pointStates = HashMap.insert point
                            PointState { checked=True
                                       , val=x
                                       } $ pointStates board
            }

wonWithNumber :: Int -> Board -> Bool
wonWithNumber num board =
  case numPoints board !? num of
    Nothing     -> False
...
isChecked board point = checked $ pointStates board ! point
...
uncheckedVals board =
  val <$> filter (not . checked) (HashMap.elems $ pointStates board)
